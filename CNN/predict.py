...
tf.flags.DEFINE_string("no_data_file", "CNN/data/no_test.txt", "Data source for the no data.")
tf.flags.DEFINE_string("yes_data_file", "CNN/data/yes_test.txt", "Data source for the yes data.")

# Eval Parameters
tf.flags.DEFINE_integer("batch_size", 64, "Batch Size (default: 64)")
# 填写训练获得模型的存储位置
tf.flags.DEFINE_string("checkpoint_dir", "CNN/runs/14/1592787213/checkpoints", "Checkpoint directory from training run")
tf.flags.DEFINE_boolean("eval_train", True, "Evaluate on all training data")#是否在训练集上评估
...
if FLAGS.eval_train:
    x_raw, y_test = data_helpers.load_data_and_labels(FLAGS.no_data_file, FLAGS.yes_data_file)
...
for i in y_test:
    if i==0.0:
        real_labels.append("yes")
    else:
        real_labels.append("no")


df = pd.DataFrame({'text':x_raw, "real_label": real_labels , "predict_label": y})
df.to_csv('CNN/data/eda_predict_result_CNN_test.csv', index=False)
